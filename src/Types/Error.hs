...
data TypeError where
  ...
  -- ^ This pattern declares one variable multiple times
  NonLinearRecord :: Expr Parsed -> T.Text -> TypeError
  -- ^ This record declares an entry multiple times

  ModuleNonFunctor :: ModuleType Typed -> ModuleTerm Resolved -> TypeError
  ModuleMismatch :: ModuleType Typed -> Var Parsed -> TypeError

  EmptyMatch :: TypeError -- ^ This @match@ has no patterns
  EmptyBegin :: TypeError -- ^ This @begin@ block has no expressions
  IllegalMethod :: TypeError -- ^ An illegal method within an @instance@
...
-- | Why this type was instantiated
data WhyInstantiate = Expression | Subsumption

-- | Why this term is unsatisfiable
data WhyUnsat
  = NotAFun
  | PatBinding
  | RecursiveDeduced
...
-- | The sort of variable we're looking up.
data VarKind
  = VarVar
  | VarCtor
  | VarType
...
instance Note TypeError Style where
  ...
  formatNote f (ArisingFrom x@NonLinearPattern{} _) = formatNote f x
  formatNote f x@(NonLinearPattern _ ps) = indent 2 (Right <$> pretty x) <#> f (map annotation ps)

  formatNote f (ArisingFrom x@NonLinearRecord{} _) = formatNote f x
  formatNote f x@(NonLinearRecord e _) = indent 2 (Right <$> pretty x) <#> f [annotation e]
